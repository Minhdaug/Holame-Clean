...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    Task.Run(() => WebBanHangOnline.Common.Common.SendMail("Holame Clean", "Đơn hàng mới từ Holame Clean: Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email));
                    ...
                    Task.Run(() => WebBanHangOnline.Common.Common.SendMail("Holame Clean", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]));
                    cart.ClearCart();
                    code = new { Success = true, Code = req.TypePayment, Url = "/ShoppingCart/CheckOutSuccess" };
                    //var url = "";
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult AddToCart(int id, int quantity, string fragrance)
        {
            ...
            if (checkProduct != null)
            {
                ...
                // Tạo mới một item giỏ hàng với thông tin sản phẩm và fragrance
                ...
                // Thêm vào giỏ (có kiểm tra gộp nếu trùng id + fragrance)
        ...
        [HttpPost]
        [AllowAnonymous]
        public ActionResult Update(int id, string fragrance, int quantity)
        {
            try
            {
                ShoppingCart cart = (ShoppingCart)Session["Cart"];
                if (cart != null)
                {
                    cart.UpdateQuantity(id, fragrance, quantity);
                    Session["Cart"] = cart;
                    return Json(new { Success = true });
                }
                return Json(new { Success = false, msg = "Cart null" });
            }
            catch (Exception ex)
            {
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult Delete(int id, string fragrance)
        {
            var code = new { Success = false, msg = "", code = -1, Count = 0 };
